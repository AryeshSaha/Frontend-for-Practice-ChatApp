...
const UpdateGroupChatModal = ({ fetchAgain, setFetchAgain, fetchMessages }) => {
  ...
  const { url, user, selectedChat, setSelectedChat } = ChatState();
  ...
  const handleRemove = async (user1) => {
    if (
      selectedChat.grpAdmin.email !== user.email &&
  ...
  const addToGroup = async (user1) => {
    if (selectedChat.users.find((u) => u.email === user1.email)) {
      toast({
    ...
    if (selectedChat.grpAdmin.email !== user.email) {
      toast({
        ...
        <ModalContent>
          ...
          <ModalBody display="flex" flexDir="column" alignItems="center">
            ...
            <Box w="100%" display="flex" flexWrap="wrap" pb={3}>
              {selectedChat.users
                .filter((u) => u.email !== selectedChat.grpAdmin.email)
                .map((u) => (
                  <UserBadgeItem
                    key={u._id}
                    user={u}
